...
OUTPUT_JSON = "/home/psp/ros_ws/src/PDDL/vlm_training_data/dataset_pointer.json"
...
def detect_object_point(image_path, color_name):
    ...
    # Return center point [x_center, y_center]
...
def main():
    ...
    for split in ["train", "val", "test"]:
        ...
        for img_data in data[split]:
            ...
            # Pre-compute valid object points
            valid_objects = []
            for obj in objects:
                color = obj.get("color")
                if color:
                    point = detect_object_point(image_path_abs, color)
                    if point:
                        obj["real_point"] = point
            ...
            for _ in range(100):
                ...
                if action_type == "home":
                    answer_dict = {
                        "objects": [{"id": "Home", "type": "target", "point": [0, 0]}],
                        "goal": "fly_to Home"
                    }
                else:
                    answer_dict = {
                        "objects": [{"id": target_obj["id"], "type": "target", "point": target_obj["real_point"]}],
    ...
    print(f"Generated {len(all_samples)} samples in {OUTPUT_JSON}")
